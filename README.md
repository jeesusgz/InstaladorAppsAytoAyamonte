...
🌐 ONLINE — vía winget, con detección de actualizaciones disponibles
...
└── README.md
⚙️ Puesta en marcha
Copia catalog.example.json como catalog.json y rellena los datos reales de tu organización (servidores, API keys, etc. — nunca subas este fichero con datos reales a un repositorio)
Crea la carpeta apps/ con una subcarpeta por programa, conteniendo el instalador correspondiente
Ejecuta con:
powershell
   powershell -ExecutionPolicy Bypass -File Launcher_v3.ps1
Para compilar a .exe (requiere el módulo ps2exe):
powershell
